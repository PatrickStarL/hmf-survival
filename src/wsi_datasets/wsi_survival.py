...
_DEFAULT_SIGNATURE_DIR = os.path.join(_SRC_DIR, 'data_csvs', 'rna', 'metadata')
...
class WSISurvivalDataset(Dataset):
    ...
    def init_df(self, df, data_source, target_transform, sample_col, slide_col,
        ...
        # Allow tiny debug splits that may only contain one class (0 or 1),
        # but still enforce binary censorship labels.
        if not set(censorship_vals).issubset({0, 1}):
    ...
    def set_feat_paths_in_df(self):
        ...
        self.feats_df[self.slide_col] = self.feats_df[self.slide_col].astype(str)
        norm_col = f"{self.slide_col}_norm"
        self.data_df[norm_col] = self.data_df[self.slide_col].str.lower()
        self.feats_df[norm_col] = self.feats_df[self.slide_col].str.lower()
        missing_mask = ~self.data_df[norm_col].isin(set(self.feats_df[norm_col]))
        missing_feats_in_split = self.data_df.loc[missing_mask, self.slide_col].drop_duplicates().reset_index(drop=True)
        ...
        try:
            assert self.feats_df[norm_col].duplicated().sum() == 0
            self.data_df = self.data_df.merge(
                self.feats_df[[norm_col, 'fpath']],
                how='left',
                on=norm_col,
                validate='1:1',
            )
            ...
            print(self.feats_df[self.feats_df[norm_col].duplicated()].to_string())
            sys.exit()

        self.data_df = self.data_df.drop(columns=[norm_col])
...
class WSIOmicsSurvivalDataset(WSISurvivalDataset):
    ...
    def _setup_func_genes(self, signature_path=None):
        r"""
        ...
        if signature_path is None:
            signature_path = _DEFAULT_SIGNATURE_DIR
    ...
    def _setup_pathways(self, type_of_path='hallmark', signature_path=None):
        r"""
    ...
    def __getitem__(self, idx):
        ...
        if self.omics_modality == 'functional':
            for oidx in range(6):
                omics_list.append(_to_float_tensor(self.omics_data.loc[case_id, self.omic_names[oidx]]))

        elif self.omics_modality == 'pathway':
            for i in range(len(self.omic_names)):
                omics_list.append(_to_float_tensor(self.omics_data.loc[case_id, self.omic_names[i]]))

        elif self.omics_modality =='no_proto_omics':
            omics_list.append(_to_float_tensor(self.omics_data.loc[case_id]))
        else:
...
class WSIOmicsTextSurvivalDataset(WSIOmicsSurvivalDataset):
    ...
    def __getitem__(self, idx):
        ...
        case_path = os.path.join(self.reports_dir, f'{case_id}.pt')
        text_embeds = torch.load(case_path, weights_only=False)
