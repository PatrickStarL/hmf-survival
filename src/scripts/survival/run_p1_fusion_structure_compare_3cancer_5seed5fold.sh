...
RUN_ROOT="${RUN_ROOT:-/path/to/your/data_root/formal_p1_fusion_structure_compare_3cancer_5seed5fold_${TS}}"
...
read -r -a STRUCTURES <<< "${STRUCTURE_LIST}"
...
if [[ ! -f "${STATUS_TSV}" ]]; then
  printf "timestamp_utc\tstatus\tstructure\tdataset\tseed\tfold\ttask\tc_index_test\tsummary_csv\tlog_file\tnote\n" > "${STATUS_TSV}"
fi
...
if [[ "${PREP_CRC_ALIAS}" == "1" ]] && contains_word "CRC" "${DATASETS[@]}"; then
  echo "[p1-structure] preparing CRC casefold alias: ${CRC_ALIAS_VISION_DIR}"
...
run_root=${RUN_ROOT}
profile=P1_fusion_structure_compare_retained_mainline_3cancer
notes=Strict formal P1 fusion-structure comparison for LUAD/STAD/CRC. All three structures run through the same twostage training entry and share the same retained-mainline preprocessing, pathway token compression, contrastive setup, and optimization protocol. The only intentional change is model_mm_type in {twostage_cls, coattn_text_twostage, cls_transformer_twostage}.
...
resolve_structure() {
  local structure="$1"
  case "${structure}" in
    twostage|two_stage|two-stage)
      STRUCTURE_KEY="twostage"
      STRUCTURE_LABEL="twostage"
      ENTRY_MODULE="src.training.main_survival_hierarchical_fusion"
      MODEL_MM_TYPE="twostage_cls"
      EXP_CODE="p1_twostage_formal"
      SUPPORTS_PATHWAY_TOKEN_COUNT="1"
      SUPPORTS_CLS_ARGS="1"
      STRUCTURE_NOTE="strict_structure_only_twostage_cls"
      ;;
    coattn_text|coattn)
      STRUCTURE_KEY="coattn_text"
      STRUCTURE_LABEL="coattn_text"
      ENTRY_MODULE="src.training.main_survival_hierarchical_fusion"
      MODEL_MM_TYPE="coattn_text_twostage"
      EXP_CODE="p1_coattn_text_formal"
      SUPPORTS_PATHWAY_TOKEN_COUNT="1"
      SUPPORTS_CLS_ARGS="1"
      STRUCTURE_NOTE="strict_structure_only_coattn_text_twostage"
      ;;
    shallow_cls|shallow-cls|cls_transformer)
      STRUCTURE_KEY="shallow_cls"
      STRUCTURE_LABEL="shallow_cls"
      ENTRY_MODULE="src.training.main_survival_hierarchical_fusion"
      MODEL_MM_TYPE="cls_transformer_twostage"
      EXP_CODE="p1_shallow_cls_formal"
      SUPPORTS_PATHWAY_TOKEN_COUNT="1"
      SUPPORTS_CLS_ARGS="1"
      STRUCTURE_NOTE="strict_structure_only_cls_transformer_twostage"
      ;;
    *)
      echo "[ERROR] Unknown structure: ${structure}" >&2
      return 1
      ;;
  esac
}
...
skipped=0
failed=0
for _structure in "${STRUCTURES[@]}"; do
...
run_one() {
  ...
  local task="${dataset}_${STRUCTURE_KEY}_panther_histotext_k${fold}_s${seed}"
  local result_root="${RUN_ROOT}/results/${STRUCTURE_KEY}/${dataset}/seed_${seed}/k_${fold}"
  ...
  if [[ "${SKIP_EXISTING}" == "1" && -n "${existing}" ]]; then
    ...
    printf "%s\tSKIP\t%s\t%s\t%s\t%s\t%s\tNA\t%s\t%s\tresume_skip_existing\n" \
      "$(date -u '+%F %T UTC')" "${STRUCTURE_LABEL}" "${dataset}" "${seed}" "${fold}" "${task}" "${existing}" "${log_file}" >> "${STATUS_TSV}"
    return 0
  fi
  ...
  if [[ "${SUPPORTS_PATHWAY_TOKEN_COUNT}" == "1" ]]; then
    cmd+=(--pathway_token_count "${PATHWAY_TOKEN_COUNT}")
  fi

  if [[ "${SUPPORTS_CLS_ARGS}" == "1" ]]; then
    cmd+=(
      --cls_depth "${CLS_DEPTH}"
  ...
  if [[ ${rc} -eq 0 && -n "${summary_csv}" ]]; then
    ...
    printf "%s\tDONE\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
      "$(date -u '+%F %T UTC')" "${STRUCTURE_LABEL}" "${dataset}" "${seed}" "${fold}" "${task}" "${cidx}" "${summary_csv}" "${log_file}" "${STRUCTURE_NOTE}" >> "${STATUS_TSV}"
    echo "[$(date -u '+%F %T UTC')] DONE ${task} c_index=${cidx}" | tee -a "${log_file}"
  else
    failed=$((failed + 1))
    printf "%s\tFAIL\t%s\t%s\t%s\t%s\t%s\tNA\t%s\t%s\t%s;rc=%s\n" \
      "$(date -u '+%F %T UTC')" "${STRUCTURE_LABEL}" "${dataset}" "${seed}" "${fold}" "${task}" "${summary_csv}" "${log_file}" "${STRUCTURE_NOTE}" "${rc}" >> "${STATUS_TSV}"
  ...
  for dataset in "${DATASETS[@]}"; do
    for seed in "${SEEDS[@]}"; do
      for fold in "${FOLDS[@]}"; do
        run_one "${structure}" "${dataset}" "${seed}" "${fold}"
...
structure_scores = defaultdict(list)

for row in rows:
    key = (row["structure"], row["dataset"])
    if row["status"] == "DONE":
...
for row in done_rows:
    ...
    if score is not None:
        group_scores[(row["structure"], row["dataset"])].append(score)
        structure_scores[row["structure"]].append(score)
...
by_structure_dataset = []
for key in sorted(group_counts.keys()):
    structure, dataset = key
    ...
    by_structure_dataset.append(entry)

by_structure = []
for structure in sorted({r["structure"] for r in rows}):
    stat = summarize_scores(structure_scores[structure])
    done = sum(1 for r in done_rows if r["structure"] == structure)
    fail = sum(1 for r in fail_rows if r["structure"] == structure)
    skip = sum(1 for r in skip_rows if r["structure"] == structure)
    by_structure.append({
        "structure": structure,
        "done": done,
        "fail": fail,
...
payload = {
    ...
    "by_structure_dataset": by_structure_dataset,
    "fold_results": sorted(
        fold_items,
        key=lambda x: (x["structure"], x["dataset"], x["seed"], x["fold"]),
    ),
}

(report_root / "cv_summary_structure_compare.json").write_text(
...
for item in payload["by_structure"]:
    lines.append(
        f"| {item['structure']} | {item['done']} | {item['fail']} | {item['skip']} | "
...
for item in payload["by_structure_dataset"]:
    lines.append(
        f"| {item['structure']} | {item['dataset']} | {item['done']} | {item['fail']} | {item['skip']} | "
        f"{item['mean'] if item['mean'] is not None else 'NA'} | "
        f"{item['std'] if item['std'] is not None else 'NA'} |"
    )
...
lines.extend([
    ...
    "## Fold Results",
    "",
    "| Structure | Dataset | Seed | Fold | c-index(test) | Summary | Log | Note |",
    "| --- | --- | ---: | ---: | ---: | --- | --- | --- |",
])
for item in payload["fold_results"]:
    lines.append(
        f"| {item['structure']} | {item['dataset']} | {item['seed']} | {item['fold']} | "
        f"{item['c_index_test'] if item['c_index_test'] is not None else 'NA'} | "
        f"{item['summary_csv']} | {item['log_file']} | {item['note']} |"
    )

(report_root / "cv_summary_structure_compare.md").write_text(
    "\n".join(lines) + "\n",
    encoding="utf-8",
)
...

{
  ...
  echo "summary_json=${REPORT_ROOT}/cv_summary_structure_compare.json"
  echo "summary_md=${REPORT_ROOT}/cv_summary_structure_compare.md"
