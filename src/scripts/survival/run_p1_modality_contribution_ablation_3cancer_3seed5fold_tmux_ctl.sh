...
SELF_SCRIPT="${SELF_SCRIPT:-/path/to/hmf-survival/src/scripts/survival/run_p1_modality_contribution_ablation_3cancer_3seed5fold_tmux_ctl.sh}"
PROJECT_DIR="${PROJECT_DIR:-/path/to/hmf-survival}"
RUN_SCRIPT="${RUN_SCRIPT:-${PROJECT_DIR}/src/scripts/survival/run_p1_modality_contribution_ablation_3cancer_3seed5fold.sh}"
SESSION="${SESSION:-hmf_p1_modality_contribution_ablation_3cancer_3seed5fold}"
GUARD_SESSION="${GUARD_SESSION:-hmf_p1_modality_contribution_ablation_3cancer_3seed5fold_guard}"
...
new_run_root() {
  date -u +"/path/to/your/data_root/formal_p1_modality_contribution_ablation_3cancer_3seed5fold_%Y%m%d_%H%M%S"
}
...
latest_run_root() {
  ...
  ls -dt /path/to/your/data_root/formal_p1_modality_contribution_ablation_3cancer_3seed5fold_* 2>/dev/null | head -n 1 || true
}
...
cmd_status() {
  ...
  if [[ -n "${rr}" ]]; then
    ...
    pgrep -af "run_p1_modality_contribution_ablation_3cancer_3seed5fold.sh" || true
  fi
  ...
  local summary_md="${rr}/reports/cv_summary_modality_ablation.md"
  local summary_json="${rr}/reports/cv_summary_modality_ablation.json"
  local launcher_log="${rr}/launcher.log"
...
cmd_stop() {
  ...
  pkill -f "run_p1_modality_contribution_ablation_3cancer_3seed5fold.sh" 2>/dev/null || true
  if [[ -n "${rr}" ]]; then
...
guard_env_prefix() {
  ...
  prefix+=" PROJECT_DIR=$(shell_quote "${PROJECT_DIR}")"
