...
try:
    ...
    def concordance_index_censored(event_observed, event_time, estimate, tied_tol=1e-8):
        """
        Lightweight fallback c-index implementation compatible with sksurv signature.
        Returns a tuple where index 0 is c-index.
        """
        ...
        cindex = (concordant + 0.5 * ties) / permissible if permissible > 0 else 0.5
        return (cindex, concordant, permissible, ties, 0.0)

from src.mil_models.tokenizer import PrototypeTokenizer
from src.mil_models.model_factory_hierarchical_fusion import create_multimodal_survival_model, prepare_emb
...
def train(datasets, args):
    ...
    valid_es_metrics = {'loss', 'train_c_index', 'val_c_index'}
    assert args.es_metric in valid_es_metrics, \
        f"Unsupported es_metric={args.es_metric}. Supported: {sorted(valid_es_metrics)}"
    ...
    # The training loop #
    #####################
    warned_val_metric_fallback = False
    ...
    for epoch in range(args.max_epochs):
        ...
        if early_stopper is not None:
            ...
            elif args.es_metric == 'train_c_index':
                score = train_results['c_index']
            elif args.es_metric == 'val_c_index':
                if val_results is None:
                    if not warned_val_metric_fallback:
                        print("Warning: es_metric=val_c_index but no VAL split found; "
                              "falling back to train c_index.")
                        warned_val_metric_fallback = True
                    score = train_results['c_index']
                else:
    ...
    for k, loader in datasets.items():
        ...
        if args.model_mm_type in ['coattn', 'coattn_text', 'cls_transformer', 'twostage_cls', 'full_twostage', 'histo_only_twostage', 'histo_text_twostage', 'histo_rna_twostage', 'cls_transformer_twostage', 'coattn_text_twostage']:
            return_attn = True
        else:
...
def normalize_attention_batch(attn, batch_size):
    """Normalize attention-like outputs so the leading dimension is always batch."""
    arr = np.asarray(attn)
    ...
    if arr.ndim == 1:
        ...
        if arr.shape[0] == batch_size:
            return arr.reshape(batch_size, 1, 1)
        return arr.reshape(1, -1, 1)

    if arr.ndim == 2:
        # Shallow-CLS pooling weights are batch-first [B, T], while attention maps
        # from batch_size=1 often arrive as [Q, K] after squeeze().
        if arr.shape[0] == batch_size:
            return arr[..., None]
        if batch_size == 1:
            return arr[None, ...]
...
def validate_survival(model, loader,
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if return_attn:
            ...
            all_omic_attn.append(
                normalize_attention_batch(out['omic_attn'].detach().cpu().numpy(), batch_size)
            )
            all_cross_attn.append(
                normalize_attention_batch(out['cross_attn'].detach().cpu().numpy(), batch_size)
            )
            all_path_attn.append(
                normalize_attention_batch(out['path_attn'].detach().cpu().numpy(), batch_size)
            )
        # End of iteration survival-specific metrics to calculate / log
        bag_size_meter.update(data.size(1), n=len(data))
        for key, val in log_dict.items():
    ...
    all_event_times = np.concatenate(all_event_times).squeeze(1)
    if return_attn:
        all_omic_attn = np.concatenate(all_omic_attn, axis=0)
        all_cross_attn = np.concatenate(all_cross_attn, axis=0)
        all_path_attn = np.concatenate(all_path_attn, axis=0)
